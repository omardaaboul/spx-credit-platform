...
function payloadFixture(): DashboardPayload {
  type MultiDteRecommendation = NonNullable<DashboardPayload["multiDte"]>["targets"][number]["recommendation"];
  const recommendation = {
    ...
    em_1sd: 70,
    iv_atm: 0.2,
  } as unknown as MultiDteRecommendation;

  return {
    generatedAtEt: "10:00:00",
    generatedAtParis: "16:00:00",
    market: { isOpen: true, hoursEt: "09:30-16:00 ET", source: "test", telegramEnabled: false },
    metrics: { spx: 5000, emr: 50, vix: 18, vwap: 4996, range15mPctEm: 0.2, atr1m: 3, putCallRatio: null, iv: 0.2 },
    regimeSummary: {
      regime: "TREND_UP",
      favoredStrategy: "2-DTE Credit Spread",
      reason: "",
    },
    ...
    dataContract: {
      ...
      checkedAtEt: "10:00:00",
      issues: [],
      feeds: {},
    },
    startupHealth: {
      telegram: { ok: true, label: "ok", detail: "ok" },
      tastySdk: { ok: true, label: "ok", detail: "ok" },
      tastyCredentials: { ok: true, label: "ok", detail: "ok" },
    },
